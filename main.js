...
async function greetUser() {
    console.log("Welcome to our typing speed tester! Ready to see how fast you can type?");
    await welcome();
}
...
async function logIn() {
    ...
    if (findIndexOfUser !== -1) {
        let isInclude = users[findIndexOfUser].email.includes(userEmail) &&
...
Remember, accuracy is just as important as speed in this medium level test.
  ...
  `);
                let text = "Throughout history, technological advancements have revolutionized the way we live, work, and communicate. From the invention of the printing press to the rise of the internet, technology has constantly pushed boundaries and transformed our world. As we continue to innovate, it's crucial to consider the ethical implications of these advancements and ensure they benefit humanity as a whole";
                console.log(`  ----------------- Text ------------------ `);
                await startTest(3, text);
            }
    ...
    \n\t`);
                let text = "Effective teamwork is essential for achieving success in many aspects of life. By combining diverse perspectives, skills, and experiences, teams can tackle complex challenges and generate innovative solutions. Strong communication, collaboration, and mutual respect are all key ingredients to fostering a productive and positive team environment. When individuals come together with a shared goal, teamwork can truly unlock remarkable potential.";
                console.log(`  ----------------- Text ------------------ `);
                await startTest(3, text);
            }
...
This is your chance to showcase your advanced typing abilities. Focus on maintaining a good pace while striving for accuracy, but don't be discouraged if you encounter unfamiliar words.
        ...
        if (option == "View Score") {
            ...
            if (score == true) {
                ...
                const findIndexOfuser = users.findIndex(user => user.name == username);
...
async function startTest(timer, text) {
    ...
    console.log(`You can approximately write ${userText.length / timer} words per minute`);
    console.log(`You write ${correctWords} correct words.`);
    console.log(`You made ${mistakes} mistakes.`);
}
async function time(timer) {
    ...
    let interval = setInterval(() => {
        ...
        process.stdout.write("\r" + `${minutes.toString().padStart(2, "0")}:${second.toString().padStart(2, "0")}`);
        if (timeDiff <= 0) {
